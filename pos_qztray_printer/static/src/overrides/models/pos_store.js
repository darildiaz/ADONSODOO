    ...
    afterProcessServerData() {
        ...
        return super.afterProcessServerData(...arguments).then(function () {
            if (self.config.other_devices && self.config.epson_printer_ip) {
                self.hardwareProxy.printer = new EpsonPrinter({ ip: self.config.qztray_printer_ip });
            }
        });
    },
    create_printer(config) {
        if (config.printer_type === "qztray_epos") {
            return new EpsonPrinter({ ip: config.qztray_printer_ip });
        } else {
